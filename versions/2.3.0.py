"""
2.3.0
- Attempt at overhauling resizing (still broken)
- Added proper cursor hints for resize edges
- Added "Auto-hide and return to game" hotkey feature
- Added setting to toggle the auto-hide behavior
"""
import sys
import threading
import queue
...
from PyQt6.QtCore import (
    Qt, QTimer, pyqtSignal, QObject, QThread, QRect, QAbstractNativeEventFilter, QEvent
)
...
class ResizableBorderWidget(QWidget):
    ...
    Handles manual resizing completely in Python to avoid buggy native OS integrations.
    """
...
class SettingsDialog(QDialog):
    def __init__(self, config: dict, themes: dict, parent=None):
        ...
        self.auto_hide_cb = QCheckBox("Auto-hide && return focus to game after sending via hotkey") # again, double ampersand
...
class ChatWindow(QMainWindow):
    ...
    def _on_hotkey(self):
        ...
        if os.name == "nt":
            ...
            # Save the currently active window so we can return focus to it later
            try:
    ...
    def _post_send_logic(self):
        ...
        # since the hotkey brought it up, auto-hide and return focus.
        if self.config.get("auto_hide_on_send", False) and self._opened_via_hotkey_only:
            self.hide()
            
            if os.name == "nt" and self._prev_hwnd:
                try:
                    # Slight delay helps Windows process the foreground change reliably
