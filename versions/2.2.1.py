...
# Needed for the "focus hotkey" feature and for actually intercepting the key.
try:
    import keyboard
...
from PyQt6.QtCore import (
    Qt, QTimer, pyqtSignal, QObject, QThread, QRect, QAbstractNativeEventFilter
)
...
def normalize_hotkey(text: str) -> str:
    """
    Normalise captured Qt hotkey text into something the `keyboard` package
    understands better.
    """
    if not text:
        return ""
...
def force_foreground_window(hwnd: int):
    """
    Windows-only helper to force a window to the foreground.
    This helps avoid the orange taskbar flash.
    """
    ...
    try:
        ...
        # If Windows still refuses, use the old Alt trick and retry.
        if not ok:
            # VK_MENU = Alt
...
# region Log Parsing and Token Management
...
class ChatClient:
    ...
    def _watch_logs(self, interval: float = 3.0):
        ...
        while self._running:
            ...
            # UI hook for re-showing the window on server switch.
...
# region PyQt UI Components

class HotkeyEdit(QLineEdit):
    """
    A capture box for hotkeys.
    Click it, then press the key/combo you want.
    Esc clears it.
    """
    ...
    def keyPressEvent(self, event):
        ...
        # Let Tab still move focus.
        if key in (Qt.Key.Key_Tab, Qt.Key.Key_Backtab):
            super().keyPressEvent(event)
            return

        # Esc clears when pressed alone.
        ...
        # Prefer the key itself, not the potentially control-character text.
        try:
            key_name = (
        ...
        # Fallback for keys where QKeySequence gives nothing useful.
        if not key_name:
            text = event.text()
        ...
        # "+" is also the hotkey separator, so use the keyboard-lib-friendly name.
        if key_name == "+":
            key_name = "plus"
...
class ChatScrollArea(QScrollArea):
    def __init__(self, theme: dict):
        ...
        self.layout.addStretch(1)  # Keeps messages bottom-aligned.
...
class ResizableBorderWidget(QWidget):
    """
    Transparent outer widget that provides a real resize border for a frameless window.

    The visible UI panel is inset inside this widget, leaving an invisible
    click/resize margin around the edge.
    """
    ...
    def manual_resize(self, global_pos):
        ...
        if self.resize_edges is None or self.drag_pos is None or self.start_geo is None:
            return

        delta = global_pos - self.drag_pos
        geo = QRect(self.start_geo)

        min_w = max(w.minimumWidth(), 220)
        min_h = max(w.minimumHeight(), 180)

        start_x = self.start_geo.x()
        start_y = self.start_geo.y()
        start_w = self.start_geo.width()
        start_h = self.start_geo.height()

        if self.resize_edges & Qt.Edge.RightEdge:
            geo.setWidth(max(min_w, start_w + delta.x()))

        if self.resize_edges & Qt.Edge.BottomEdge:
            geo.setHeight(max(min_h, start_h + delta.y()))

        if self.resize_edges & Qt.Edge.LeftEdge:
            new_left = min(start_x + delta.x(), start_x + start_w - min_w)
            geo.setLeft(new_left)

        if self.resize_edges & Qt.Edge.TopEdge:
            new_top = min(start_y + delta.y(), start_y + start_h - min_h)
            geo.setTop(new_top)
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if edges is not None:
                ...
                window_handle = w.windowHandle()

                # Prefer native interactive resize if available.
                if window_handle is not None and hasattr(window_handle, "startSystemResize"):
                    try:
                        if window_handle.startSystemResize(edges):
                            event.accept()
                            return
                    except Exception:
                        # Fall back to manual resizing if the native call fails.
                        pass

                # Fallback manual resize.
                self.resize_edges = edges
                self.drag_pos = event.globalPosition().toPoint()
                self.start_geo = w.frameGeometry()
                self.grabMouse()
                event.accept()
                return
    ...
    def mouseMoveEvent(self, event):
        if self.resize_edges is not None:
            self.manual_resize(event.globalPosition().toPoint())
            return

        self.update_cursor(self.edge_at(event.position().toPoint()))
...
if os.name == "nt":
    ...
    class FramelessResizeFilter(QAbstractNativeEventFilter):
        """
        Windows-only native event filter that gives a frameless window
        proper native resize borders.
        """

        def __init__(self, window, border: int = 12):
            super().__init__()
            self._window = window
        ...
        def nativeEventFilter(self, event_type, message):
            try:
                ...
                if self._hwnd_value(msg.hwnd) != self._hwnd:
                    return False, 0
                ...
                if msg.message == WM_NCHITTEST:
                    ...
                    # Extract exact screen coordinates directly from lParam
                    ...
                    # Use <= and >= to ensure the exact edge/corner pixels are included
                    left = pos_x <= b
                    right = pos_x >= w - b
                    top = pos_y <= b
                    bottom = pos_y >= h - b
...
class ChatWindow(QMainWindow):
    ...
    def __init__(self, client: ChatClient, config: dict):
        ...
        # Outer transparent resize border.
        outer = ResizableBorderWidget(self)
        outer.setStyleSheet("background-color: rgba(0, 0, 0, 1);")
        self.setCentralWidget(outer)

        outer_layout = QVBoxLayout(outer)
        outer_layout.setContentsMargins(
            ResizableBorderWidget.BORDER,
            ResizableBorderWidget.BORDER,
            ResizableBorderWidget.BORDER,
            ResizableBorderWidget.BORDER
        )
        ...
        self.hotkey_pressed.connect(lambda: self.show_from_tray(focus_input=True))
    ...
    def _on_hotkey(self):
        # This runs in the keyboard hook thread.
        # Do the Win32 foreground force here, then let Qt handle focus safely.
        if os.name == "nt":
            self._ensure_hwnd()
    ...
    def show_from_tray(self, focus_input: bool = False):
        if self.isMinimized():
            self.showNormal()
    ...
    def open_settings(self):
        # Temporarily unregister while capturing a new hotkey.
        self.unregister_hotkey()

        dialog = SettingsDialog(self.config, THEMES, self)
        dialog.exec()

        # Config is only changed if Save was pressed.
        self.register_hotkey()
...
if __name__ == "__main__":
    ...
    if os.name == "nt" and FramelessResizeFilter is not None:
        try:
            resize_filter = FramelessResizeFilter(
                window,
                border=ResizableBorderWidget.BORDER + int(ResizableBorderWidget.BORDER/5)
            )
            app.installNativeEventFilter(resize_filter)

            # Important: keep a reference so it is not garbage collected.
            window._native_resize_filter = resize_filter

        except Exception as e:
            print(f"Native resize filter unavailable: {e}")
